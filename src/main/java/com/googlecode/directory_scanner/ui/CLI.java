...
public class CLI {
	...
	public CLI(String[] args, Logger logger, Properties config,
			WorkManager worker) {
		...
		for (String s : args) {
			...
			if (command == null) {
				if ("scan".equals(s) || "rescan".equals(s)
						|| "check_exists".equals(s)) {
					command = s;
				...
				if ("scan".equals(command)) {
					...
					logger.log(
							Level.ERROR,
							"unhandled stated, please share the call that caused this output with the developers!");
				}
				command = null;
			}
