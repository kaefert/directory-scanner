...
public class GUI {
	...
	JMenuItem np = new JMenuItem(new AbstractAction("New / Other Profile") {
		...
		String s = (String) JOptionPane.showInputDialog(frmDirectoryScanner, "Enter the Profile-Name:", "Customized Dialog", JOptionPane.PLAIN_MESSAGE);
	...
	JMenuItem move = new JMenuItem(new AbstractAction("Move Files that exist below Path1&2 from Path2% into Path3%") {
	    private static final long serialVersionUID = -2431148565166087751L;

	    @Override
	    public void actionPerformed(ActionEvent e) {
		autoscan(false, new int[] {1,2});
		BlockingQueue<ReportMatch> queue = worker.findFiles(txtPath1.getText(), txtPath2.getText(), true, getReportSortMode());
		worker.moveOrCopyMatches(queue, txtPath2.getText(), txtPath3.getText(), false, flattenTarget.isSelected());
	    }
	});
	move.setMnemonic('m');
	actionsMenu.add(move);

	JMenuItem moveNeg = new JMenuItem(new AbstractAction("Move Files that don't exist below Path1 from Path2% into Path3%") {
	    private static final long serialVersionUID = -2431148565166087751L;

	    @Override
	    public void actionPerformed(ActionEvent e) {
		autoscan(false, new int[] {1,2});
		BlockingQueue<ReportMatch> queue = worker.findFiles(txtPath2.getText(), txtPath1.getText(), false, getReportSortMode());
		worker.moveOrCopyMatches(queue, txtPath2.getText(), txtPath3.getText(), false, flattenTarget.isSelected());
	    }
	});
	...
	JMenuItem copy = new JMenuItem(new AbstractAction("Copy Files that exist below Path1&2 from Path2% into Path3%") {
	    private static final long serialVersionUID = -2431148565166087751L;
		...
		autoscan(false, new int[] {1,2});
		BlockingQueue<ReportMatch> queue = worker.findFiles(txtPath1.getText(), txtPath2.getText(), true, getReportSortMode());
		worker.moveOrCopyMatches(queue, txtPath2.getText(), txtPath3.getText(), true, flattenTarget.isSelected());
	    }
	});
	copy.setMnemonic('m');
	actionsMenu.add(copy);

	JMenuItem copyNeg = new JMenuItem(new AbstractAction("Copy Files that don't exist below Path1 from Path2% into Path3%") {
	    private static final long serialVersionUID = -2431148565166087751L;
